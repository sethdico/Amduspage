...
module.exports.run = async function ({ event, args, api, reply }) {
  ...
  if (currentImg && !userPrompt) return reply("🖼️ I see your image! Reply to it with instructions like 'describe this'");
  if (repliedImg && !userPrompt) return reply("📝 Please add your instruction!");
  ...
  try {
    ...
    if (match) {
      ...
      if (parseInt(fileRes.headers['content-length'] || '0') > MAX_FILE_SIZE) return reply("⚠️ file too large");
