...
module.exports.run = async function ({ event, args }) {
  ...
  // Special AI note
  const aiNote = "No need to use this command since it automatically answers your questions without a command name.";
  ...
  try {
    // Organize commands (Exclude Admin)
    const categories = {};
    ...
    // 1. FOLDER VIEW (Checking this first to fix the "Utility" issue)
    ...
    if (matchedCat) {
      ...
      categories[matchedCat].sort((a, b) => a.name.localeCompare(b.name)).forEach(cmd => {
        ...
        const desc = cmd.name === "ai" ? aiNote : (cmd.description || "No description.");
      ...
      // Use sendMessage for folders because they can be too long for buttons
      return api.sendMessage(catMsg, senderID);
    }

    // 2. INDIVIDUAL COMMAND INFO
    if (input && (commands.has(input) || global.client.aliases.has(input))) {
      ...
      const msg = `🤖 **COMMAND: ${cmd.name.toUpperCase()}**\n` +
        `━━━━━━━━━━━━━━━━\n` +
        `📝 **Info:** ${cmd.name === "ai" ? aiNote : (cmd.description || "No description.")}\n` +
    ...
    menuMsg += `━━━━━━━━━━━━━━━━\n💡 Click a folder to see descriptions and usage instructions.`;
