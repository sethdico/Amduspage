...
categories[matchedCat]
        ...
        .forEach((cmd) => {
          ...
          const desc =
            cmd.name === "ai"
              ? "No need to \nuse this command since it automatically answers your questions without a command name."
    ...
    if (input && (commands.has(input) || global.client.aliases.has(input))) {
      const cmdName = commands.has(input) ?
input : global.client.aliases.get(input);
...
const msg =
        ...
        `━━━━━━━━━━━━━━━━\n` +
        `📝 **Info:** ${cmd.name === "ai" ?
"No need to use this command since it automatically answers your questions without a command name." : cmd.description ||
"No description."}\n` +
...
if (config.ADMIN?.includes(senderID) && adminCommands.length > 0) {
