...
module.exports.run = async function ({ event, args, api, reply }) {
    ...
    if (index && !isNaN(index)) {
        ...
        if (!list) return reply("type getuser first to see the list");
        ...
        msg += `messages: ${user.count || 0}\n`;
        msg += `last seen: ${new Date(user.lastActive).toLocaleDateString()}`;
    ...
    try {
        ...
        if (others.length === 0) return reply("nobody else is active");

        const updated = await Promise.all(
            others.map(async (u) => {
                // if name is generic, try to get the real one once
