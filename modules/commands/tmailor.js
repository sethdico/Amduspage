...
const SWID = "61b182e12e940e27b58c394580e4d20b";
...
module.exports.run = async ({ event, args, reply }) => {
    ...
    if (action === "inbox") {
        ...
        try {
            ...
            if (!emails || emails.length === 0) return reply("inbox empty.");

            let text = "inbox contents:\n\n";
            emails.forEach((mail, index) => {
                text += `${index + 1}. from: ${mail.sender}\nsubject: ${mail.subject}\n\n`;
            });
            return reply(text);
        } catch (e) {
            return reply("access failed.");
        }
    }
